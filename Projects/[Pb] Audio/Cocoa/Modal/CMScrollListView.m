...
@interface CMScrollListView()
{
    CGFloat _defaultColumnWidth;
    NSTableView * _tableView;
    NSColor *     _fillColor;
    
    CMProxyConnection * _dataModel;
}
...
@implementation CMScrollListView
...
-(void)deleteSelectedTableEntry:(int)row
{
    ...
    if( row < _dataModel.documents.count)
    {
        ...
        __block CMProxyConnection * thruConnectionDOM = [_dataModel.documents objectAtIndex:row];
        __block CMProxyConnection * thruConnectionKey = [_dataModel.keys objectAtIndex:row];
...
-(void)editMenuItemClicked:(id)sender
{
    if( self.tableView.clickedRow < CMClient.numThruConnections)
    {
        ...
        CMProxyConnection * thruConnection = [_dataModel.documents objectAtIndex:self.tableView.clickedRow];
        thruConnection.row = self.tableView.clickedRow;
        //[[PbAudioAppDelegate sharedInstance] openProxyConnectionModalWindow:thruConnection.ThruID];
...
{
    if( [identifier localizedCompare:CMProxyConnection.primaryKey] == NSOrderedSame )
    {
...
-(nullable NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if( self.dataModel )
    {
        ...
        if( self.dataModel == CMProxy )
        {
            CMProxyConnection * thruConnectionDOM = [CMProxy.documents objectAtIndex:row];
...
-(NSInteger)numberOfRowsInTableView:(NSTableView *)tableView
{
    ...
    {
        if( self.dataModel == CMProxy)
            return CMProxy.documents.count;
        else
            return 2;
