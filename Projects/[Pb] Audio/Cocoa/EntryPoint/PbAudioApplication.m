...
@implementation PbAudioApplication

BOOL _shouldKeepRunning = YES;

+(PbAudioApplication*)sharedInstance{
    return (PbAudioApplication*)(CocoaApplication.sharedApplication);
}
...
-(id)init
{
    ...
    if(self)
    {
        NSLog(@"PbAudioApplication::init");

#if TARGET_OS_OSX
        //Create Custom App Delegate Here To Avoid Having to Use .xib
        self.delegate = [PbAudioAppDelegate sharedInstance];
#else
