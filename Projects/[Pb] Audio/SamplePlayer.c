...
//#endif
        ...
        int32_t nSourceChannels = player->sourceAudioFile.sourceFormat.nChannels;
        int32_t nStreamChannels = 2;// player->sourceAudioFile.sourceFormat.nChannels;

        int32_t nSourceSampleBytes = player->sourceAudioFile.sourceFormat.nBlockAlign / nSourceChannels;
        ...
        //16-bit le signed integer interleaved source -> 32-bit host interleaved float
            ...
            pb_audio_transform[player->sourceAudioFile.type][target](&samplesL, &byteBufferL, nSourceChannels, samplesToCopy);
...
void SamplePlayerInit(SamplePlayer* player, const char * audioFileURL, const char * audioFileEXT, PBAStreamFormatRef converterFormat)
{
    ...
    int nSourceBuffers       = player->sourceAudioFile.sourceFormat.nChannels;
    int nInterleavedChannels = 1;
    if (converterFormat) nSourceBuffers = converterFormat->nChannels;
#else
    int nSourceBuffers       = 1;
    int nInterleavedChannels = player->sourceAudioFile.sourceFormat.nChannels;
#endif
