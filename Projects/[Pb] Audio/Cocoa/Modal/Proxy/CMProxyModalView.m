...
@interface CMProxyModalView()
{
    
    CMScrollListView * _scrollListView;
...
@implementation CMProxyModalView
...
{
    _scrollListView = [[CMScrollListView alloc] initWithFrame:CGRectMake(14,0,self.frame.size.width - 28, 100) andDOM:self.dataModel];
...
-(void) createThruIDLabel
{
    ...
    self.thruIDLabel.stringValue  = @"ProxyID:";
...
-(void)createThruIDTextField
{
    ...
    self.thruIDField.placeholderString = @"Enter ProxyID";
    //_thruIDLabel.font = [NSFont systemFontOfSize:36.0];
...
{
    NSLog(@"CMProxyModalView::buttonToggledAtIndex(%d)", index);
...
-(id)initWithFrame:(NSRect)frame andProxy:(NSString* _Nullable)thruID
{
    ...
    if(self)
    {
        ...
        {
            self.dataModel = [CMProxy.dictionary objectForKey:thruID];
        ...
        else
        {
            ...
            self.dataModel = [[CMProxyConnection alloc] init];
        }
...
-(void)createUserInputFields
{
    ...
    {
        objc_property_t *properties = class_copyPropertyList([CMProxyConnection class], &count);
...
-(void)submitButtonClicked:(id)sender
{
    ...
    if( self.modalViewMode == ModalView_Create )
    {
        ...
        [CMProxyConnection createProxyConnection:sourceKey Params:thruParams];
        [CMDevice.documents.firstObject sendMCUDeviceInitializationMessages];
    ...
    else if( self.modalViewMode == ModalView_Modify )
    {
        ...
        if( [_dataModel.ThruID localizedCompare:self.thruIDField.stringValue] != NSOrderedSame )
        {
            ...
            if([CMProxy.dictionary objectForKey:self.thruIDField.stringValue]) assert(1==0);

            [_dataModel replaceProxyConnection:self.thruIDField.stringValue atIndex:_dataModel.row];
        }
        else //but otherwise we just need to update the params on the CoreMIDI thru connection
        {
            [_dataModel saveProxyConnection];
        }
...
-(void)showAlertView:(NSString*)title message:(NSString*)message completionHandler:(void (^ __nullable)(NSModalResponse returnCode))handler
{
    ...
    //dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        //CMProxyApplicationDelegate *appDelegate = (CMProxyApplicationDelegate *)[[NSApplication sharedApplication] delegate];
