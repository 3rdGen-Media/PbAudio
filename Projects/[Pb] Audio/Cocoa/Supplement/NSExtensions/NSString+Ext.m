...
+(NSString *)stringForMIDIMessageType:(MIDIMessageType)messageType andStatus:(MIDICVStatus)status
{
    switch (messageType)
    {
        ...
        case (kMIDIMessageTypeChannelVoice1):
            return [NSString stringWithFormat:@"MIDI 1.0 Channel Voice Message (%@)", [NSString stringForMIDICVStatus:status]];
        case (kMIDIMessageTypeSysEx):
            return @"Sysex";
        case (kMIDIMessageTypeChannelVoice2):
            return [NSString stringWithFormat:@"MIDI 2.0 Channel Voice Message (%@)", [NSString stringForMIDICVStatus:status]];
        case (kMIDIMessageTypeData128):
            return [NSString stringWithFormat:@"MIDI Message Type 128 (%@)", [NSString stringForMIDICVStatus:status]];
        default:
            return @"";
    }
}
