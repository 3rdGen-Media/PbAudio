...
static void* xnz_aligned_alloc(size_t size, size_t alignment)
{
    void* data = NULL;
...
static void xnz_aligned_free(void* data)
{
#if    defined(_MSC_VER) || defined(__MINGW32__)
    _aligned_free(data);
#else
...
static void xnz_print_huff_node(xnz_huff_node* node, xnz_uint2* code_table, const char* prefix)
{
    ...
    else
    {
        ...
        char* prefixCodeStrL = (char*)alloca(strlen(prefix) + 4 + 1);  //numChars = numBits + colon + terminating character
        char* prefixCodeStrR = (char*)alloca(strlen(prefix) + 4 + 1); //numChars = numBits + colon + terminating character
...
static xnz_huff_node* xnz_add_huff_node(xnz_huff_node* tree, xnz_huff_node * root, uint32_t code_bits, uint32_t num_root_code_bits, uint32_t num_code_bits, uint32_t symbol, /*uint32_t treeDepth,*/ uint32_t* nodeCount)
{
    ...
    */
    ...
    if( num_code_bits == 0 ) //base recursion case (ie designate a leaf node for the corresponding symbol)
    {
        ...
        xnz_huff_node aNode = { symbol, 0, (struct xnz_huff_node*)NULL, (struct xnz_huff_node*)NULL };
        tree[(*nodeCount)] = aNode;
    ...
    else
    {
        ...
        if (!rootNode) //if the node passed in was null get memory for a new node
        {
            ...
            xnz_huff_node aNode = { UINT32_MAX, num_code_bits, (struct xnz_huff_node*)NULL, (struct xnz_huff_node*)NULL };
            tree[(*nodeCount)] = aNode;// (xnz_huff_node) { UINT32_MAX, num_code_bits, NULL, NULL };
...

{
    ...
    return (xnz_status)(XNZ_STATUS_OK + !(stream->ll_node->symbol - 256)); //block was either partially decoded or end of block was reached
}
...
static xnz_status xnz_inflate_fixed_block(xnz_block_stream* stream, xnz_bitstream* bs, uint32_t flags)
{
    ...
    for (; i < 144; i++) { xnz_uint2 tmp = { 8, 0b00110000  + (i      )}; ll_codes[i] = tmp; };
    for (; i < 256; i++) { xnz_uint2 tmp = { 9, 0b110010000 + (i - 144)}; ll_codes[i] = tmp; };
    for (; i < 280; i++) { xnz_uint2 tmp = { 7, 0b0000000   + (i - 256)}; ll_codes[i] = tmp; };
    for (; i < 288; i++) { xnz_uint2 tmp = { 8, 0b11000000  + (i - 280)}; ll_codes[i] = tmp; };
    ...
    for (i=0; i < N_FIXED_DIST_CODES; i++) { xnz_uint2 tmp =  { 5, i }; dist_codes[i] = tmp; }
...
static xnz_uint2* code_lengths_to_code_table(uint32_t* code_lengths, xnz_uint2* code_table, uint32_t nCodeLengths, uint32_t * max_length)
{
     ...
     ***/
        ...
        { 
            ...
            if (length != 0 )
            {
                xnz_uint2 tmp = { length, next_code[length] }; code_table[i] = tmp;
                next_code[length] += 1;
            }
        }
...
static size_t xnz_inflate_buffer(uint8_t* src_buf, size_t src_size, uint8_t* dst_buf, size_t dst_size, uint8_t * history_buffer)
{
    ...
    stream.history = history_buffer ? history_buffer : (uint8_t*)alloca(XNZ_DEFLATE_BUFFER_SIZE);
